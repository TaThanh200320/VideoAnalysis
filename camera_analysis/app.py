...
class MainApp:
    def __init__(self):
        ...
        self.SLEEP_INTERVAL = 0.02  # Set a reasonable sleep interval
        self.frame_count = {}
    ...
    async def process_camera(self, camera_id, camera_info):
        ...
        if img is not None:
            ...
            if model_id in self.models:
                ...
                elapsed = current_time - self.frame_count[camera_id]['last_time']
                if elapsed >= 10.0:
                    fps = self.frame_count[camera_id]['count'] / elapsed
                    self.frame_count[camera_id]['count'] = 0
                    self.frame_count[camera_id]['last_time'] = current_time
    ...
    async def main_loop(self):
        ...
        while True:
            ...
            # Construct the corresponding relationship between camera ID and model type
            # Get camera status every 10 second
            ...
            for camera_id_str, status in camera_status.items():
                ...
                if status["alive"] == "True" and status["last_image_timestamp"] and status["camera_info"]:
                    camera_info = json.loads(status["camera_info"])
                    # print("camera_info", camera_info)
                    if camera_info:
                        tasks.append(asyncio.create_task(self.process_camera(camera_id, camera_info)))
            # self.logger.debug(f"Get camera status in {time.monotonic() - start_time:.2f} seconds")                
            if tasks:
                # Execute all camera processing tasks asynchronously

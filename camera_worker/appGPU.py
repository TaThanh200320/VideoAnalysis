...
def get_camera_resolution_ffmpeg(camera_id, camera_url, resolution_event, resolution_data, stop_event, max_retries=3):
    ...
    while retry_count < max_retries and not stop_event.is_set():
        try:
            ...
            if resolution:
                ...
                print(f"[{camera_id}] Resolution detection: {width}x{height} for {camera_url}")
                ...
                print(f"[{camera_id}] Resolution timeout: {result}")
                print(f"[{camera_id}] Resolution detection failed, unable to obtain output: try again {retry_count + 1}/{max_retries}")
        except Exception as e:
            print(f"[{camera_id}] Resolution acquisition failed ({retry_count + 1}/{max_retries}): {e}")
    ...
    print(f"[{camera_id}] Unable to get camera resolution, please check connection settings.")
    ...
    try:
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                print(f"[{camera_id}] [GPU] Try using FFmpeg to connect to the camera.")
                print(f"[{camera_id}] [GPU] Successfully connected")
            except Exception as e:
                print(f"[{camera_id}] [GPU] Unable to start FFmpeg process: {e}")
                retry_count += 1
                sleep(2)
                continue
            ...
            while not stop_event.is_set():
                try:
                    ...
                    if not raw_frame:
                        print(f"[{camera_id}] [GPU] Unable to read image frames from FFmpeg, the connection may be interrupted.")
                        retry_count += 1
                        if retry_count >= max_retries:
                            print(f"[{camera_id}] [GPU] Maximum number of retries reached. Switch to using OpenCV.")
                            process.terminate()
                            process.wait()
                            # Call OpenCV method
                            ...
                            return
                        break
                    # print(f"[{camera_id}] camera read frame: {time.monotonic() - current_time}")   
                    # print(f"[{camera_id}] [GPU] retry_count:{retry_count}.")                
                    ...
                    # print(f"[{camera_id}] camera process frame: {time.monotonic() - current_time}")                
                    #FPS calcule TuanDA
                    frame_count += 1
                    current_time = time.monotonic()
                    ...
                    print(f"[{camera_id}] [GPU] An error occurred: {e}")
                    import traceback
                    traceback.print_exc()
                    retry_count += 1
                    if retry_count >= max_retries:
                        print(f"[{camera_id}] [GPU] has reached the maximum number of retries. Switch to using OpenCV.")
                        process.terminate()
                        process.wait()
                        # Call OpenCV method
                        fetch_frame_opencv(camera_id, camera_url, stop_event)
            ...
            if process:
                process.terminate()
                process.wait()
                print(f"[{camera_id}] [GPU] Stop acquiring image frames.")

    except Exception as e:
        print(f"[{camera_id}] [GPU] Unhandled exception: {e}")
        import traceback
        traceback.print_exc()
        stop_event.set()
...
def fetch_frame_opencv(camera_id, camera_url, stop_event):
    ...
    """
    print(f"[{camera_id}] [CPU] Connect to the camera using OpenCV. ")
    if camera_url is None:
        print(f"[{camera_id}] Camera is None url. May be is not active")
    ...
    last_time = time.monotonic()
    if not cap.isOpened():
        print(f"[{camera_id}] [CPU] Unable to open camera using OpenCV.")
        r.set(f'camera_{camera_id}_status', 'False')
        return
    ...
    while not stop_event.is_set():
        ret, frame = cap.read()
        if not ret:
            print(f"[{camera_id}] [CPU] Failed to read image using OpenCV, try to reconnect.")
            cap.release()
            time.sleep(2)
            cap = cv2.VideoCapture(camera_url)
            if not cap.isOpened():
                print(f"[{camera_id}] [CPU] Unable to reconnect camera.")
                r.set(f'camera_{camera_id}_status', 'False')
                stop_event.set()
                break
            continue
        ...
        if elapsed >= 10.0:
            ...
            last_time = current_time
    cap.release()
    print(f"[{camera_id}] [CPU] Stop using OpenCV to acquire images.")
...
def get_resolution_and_start_fetching(camera_id, camera_url, camera_fps, stop_event):
    ...
    try:
        ...
        if 'width' in resolution_data and 'height' in resolution_data:
            ...
            print(f"[{camera_id}] Unable to obtain resolution, use OpenCV direct connection instead.")
            # Call OpenCV method
            fetch_frame_opencv(camera_id, camera_url, stop_event)
    except Exception as e:
        print(f"[{camera_id}] An error occurred while getting resolution: {e}")
        import traceback
        traceback.print_exc()
        # Call OpenCV method
        fetch_frame_opencv(camera_id, camera_url, stop_event)
    ...
    with camera_threads_lock:
        ...
        for camera_id in new_camera_ids.union(existing_camera_ids):
            ...
            if  camera_info is not None:
                ...
                new_fps = camera_info.get('config').get('fps') if camera_info.get('config').get('fps') is not None else 30
            else:
                new_fps = 30    
            if camera_id in camera_threads:
                ...
                print(f"[{camera_id}] Start a new camera thread.")
                stop_event = threading.Event()
                camera_thread = threading.Thread(
                    target=get_resolution_and_start_fetching,
                    args=(camera_id, new_url, new_fps, stop_event)
        ...
        with camera_threads_lock:
            for camera_id, thread_info in list(camera_threads.items()):
                ...
                if not thread.is_alive():
                    print(f"[{camera_id}] The thread has stopped. Try restarting.")
                    # Clear stop event
                    stop_event.clear()
                    #Create new thread
